...
if tonumber(prize_pool_count) <= 0 then
    ...
    if tonumber(share_current_value) <= 0 and tonumber(invite_current_value) <= 0 then
        ...
        if tonumber(share_current_value) > 0 then
            redis.call("hset", invite_share_count_key, "share_current", share_current_value - 1)
        else
            redis.call("set", invite_count_key, invite_current_value - 1)
        end
        ...
        if pool == 2 and prize == 0 and tonumber(grand_prize_index) < tonumber(prize_pool_count) and grand_prize_time_up == "false" then
            return "[\"com.opay.invite.model.response.PrizePoolResponse\",{\"message\":\"success\",\"pool\":" .. pool .. ",\"activityCount\":"..(prize_pool_count-1)..",\"userCount\":" .. (share_current_value + invite_current_value-1) .."}]"
        end
        if (tonumber(grand_prize_index) > tonumber(prize_pool_count) or grand_prize_time_up == "true") and tonumber(redis.call("get", "prize_pool:second:0")) > 0 then
            ...
            return "[\"com.opay.invite.model.response.PrizePoolResponse\",{\"message\":\"success\",\"pool\":2,\"prize\":0,\"activityCount\":"..(prize_pool_count-1)..",\"userCount\":" .. (share_current_value + invite_current_value-1) .."}]"
        end
        ...
        if prize_count == false then
            ...
            if tonumber(prize_count) > 0 then
                ...
                return "[\"com.opay.invite.model.response.PrizePoolResponse\",{\"message\":\"success\",\"pool\":" .. pool .. ",\"prize\":" .. prize .. ",\"activityCount\":"..(prize_pool_count-1)..",\"userCount\":" .. (share_current_value + invite_current_value-1) .."}]"
            else
                return "[\"com.opay.invite.model.response.PrizePoolResponse\",{\"message\":\"success\",\"pool\":" .. pool .. ",\"activityCount\":"..(prize_pool_count-1)..",\"userCount\":" .. (share_current_value + invite_current_value-1) .."}]"
            end
        end
